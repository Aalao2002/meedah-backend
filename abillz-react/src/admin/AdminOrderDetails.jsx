...
function AdminOrderDetails() {
    ...
    useEffect(() => {
        const getOrder = async () => {
            ...
            setError(null);
                try {
                    const res = await fetch(`${API_URL}/ord/${order_id}`, {
                        method: 'GET',
                        headers: {
        ...
        return (
            <div className="max-w-550 mx-auto">
                ...
                <div className='pl-5 md:hidden'>
                    <button onClick={() => navigate("/admin")} className='hover:underline flex 
                    items-center text-sm gap-1'><ArrowLeft size={14} />back</button>
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-6">
                    ...
                    <div className="border border-[#000]/10 rounded-xl shadow-sm p-6">
                        <h2 className="font-semibold font-poppins text-[#000]/80 mb-4">Customer Details</h2>
                        <p className="text-sm text-[#000]/80"><span className='text-xs text-[#000]'>Name:</span> {customerName || '—'}</p>
                        {order.email && <p className="text-xs text-[#000]/40 mt-1"><span className='text-xs text-[#000]'>Email: </span>{order.email}</p>}
                        {order.phone && <p className="text-xs text-[#000]/40"><span className='text-xs text-[#000]'>Tel no: </span>{order.phone}</p>}
                        {order.phone && <p className="text-xs text-[#000]/40"><span className='text-xs text-[#000]'>Address: </span>{order.address}</p>}
